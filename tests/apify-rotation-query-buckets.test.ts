...
describe('Phase 3A Iran-focused Apify rotation queries', () => {
  it('keeps rotation cost-neutral with the existing six source ids and maxItems=24', async () => {
    ...
    for (const plan of result.plans) {
      expect(plan.inputOverride.maxItems).toBe(24);
  ...
  it('builds Iran-focused core news, market, token, and security buckets', async () => {
    const result = await runApifyRotation(makeEnv(), { force: true, dryRun: true });
    ...
    expect(security.cohortName).toBe('security_alert_text');
    expect(String(security.inputOverride.query)).toContain('phishing');
    expect(String(security.inputOverride.query)).toContain('"private key"');
    expect(String(security.inputOverride.query)).toContain('min_faves:15');

    const market = bySource(result, 'src_market_trending_x_text');
    expect(market.cohortName).toBe('market_impact_text');
    expect(String(market.inputOverride.query)).toContain('Tether');
    expect(String(market.inputOverride.query)).toContain('"exchange inflow"');
    expect(String(market.inputOverride.query)).toContain('min_faves:75');

    const tokenProject = bySource(result, 'src_market_trending_x_media');
    expect(tokenProject.cohortName).toBe('token_project_watch_text');
    expect(String(tokenProject.inputOverride.query)).toContain('Notcoin');
    expect(String(tokenProject.inputOverride.query)).toContain('DOGS');
    expect(String(tokenProject.inputOverride.query)).toContain('"Hamster Kombat"');
    expect(String(tokenProject.inputOverride.query)).toContain('-phishing');
    expect(String(tokenProject.inputOverride.query)).toContain('min_faves:40');
  });
});
