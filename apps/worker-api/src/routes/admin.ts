...
export async function handleAdmin(
  ...
  try {
    ...
    // ── Cancel queue item ─────────────────────────────────────
    if (path.startsWith('/internal/queue/') && m === 'DELETE') {
      const queueId = pathSegment(path, 2); // /internal/queue/{id}
      if (!isValidId(queueId)) return err('invalid queue id', 400);
      const result = await env.DB
        .prepare("UPDATE publish_queue SET status='cancelled' WHERE id=? AND status IN ('scheduled','retry')")
        .bind(queueId).run();
...
async function listItems(env: Env, url: URL): Promise<Response> {
  ...
  let q = 'SELECT id,run_id,category_id,platform,source_account,source_url,ai_score,ai_risk,ai_priority,risk_flags,status,created_at,text FROM discovery_items WHERE status=?';
...
async function listQueue(env: Env, url: URL): Promise<Response> {
  ...
  let q = 'SELECT id,item_id,channel_id,language,source_url,caption_short,telegram_method,media_urls,scheduled_at,status,retry_count,publish_error,published_at,created_at FROM publish_queue WHERE status=?';
  const params: any[] = [status];
...
async function createCategory(req: Request, env: Env): Promise<Response> {
  ...
  await env.DB.prepare(`
    ...
    (id, label, prompt_profile, score_threshold, freshness_hours, media_mode, language_targets, enabled)
    VALUES (?, ?, ?, ?, ?, ?, ?, 1)
  `).bind(
    body.id,
...
async function createChannel(req: Request, env: Env): Promise<Response> {
  ...
  const allowedWindows = validateWindowsJson(body.allowed_windows, '["08:00-00:00"]');
...
async function updateChannel(req: Request, env: Env, id: string): Promise<Response> {
  ...
  if (body.allowed_windows !== undefined) {
    fields.push('allowed_windows=?'); vals.push(validateWindowsJson(body.allowed_windows, '["08:00-00:00"]'));
  }
...
async function getStats(env: Env): Promise<Response> {
  const [categories, channels, queuePending, queuePublished, lastRun, itemsToday] = await Promise.all([
...
function validateWindowsJson(input: any, fallback: string): string {
  ...
  try {
    ...
    const valid = parsed.filter((w: any) => typeof w === 'string' && /^\d{2}:\d{2}-\d{2}:\d{2}$/.test(w));
