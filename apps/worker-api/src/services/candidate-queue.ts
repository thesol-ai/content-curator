...
export function getTranslationMaxCandidateAttempts(env: Env): number {
  const base = getMaxCandidateAttempts(env);
  const v = parseInt(env.AI_CANDIDATE_TRANSLATION_MAX_ATTEMPTS ?? String(base), 10);
  if (!Number.isFinite(v) || v <= 0) return base;
  return Math.max(base, Math.min(Math.floor(v), 10));
}
...
export async function fetchPendingCandidates(
  ...
  try {
    ...
    const translationMaxAttempts = getTranslationMaxCandidateAttempts(env);
    ...
    const binds: unknown[] = [cutoff, translationMaxAttempts, maxAttempts];
...
export async function hasPendingCandidatesForPlatform(
  ...
  try {
    ...
    const binds: unknown[] = [platform, translationMaxAttempts, maxAttempts, cutoff];
    if (categoryId) { conds.push('category_id = ?'); binds.push(categoryId); }
...
export async function claimCandidateBatch(
  ...
  for (const row of candidates) {
    try {
      ...
      `).bind(row.id, translationMaxAttempts, maxAttempts).run();
...
export async function failMaxAttemptPendingCandidates(env: Env): Promise<number> {
  try {
    ...
    return (normal.meta.changes ?? 0) + (translation.meta.changes ?? 0);
  } catch (err) {
