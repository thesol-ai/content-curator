...
const LOCAL_STOP_TERMS = new Set([
  ...
  'says','said','per','according','crypto','cryptocurrency','market','markets','blockchain',
  'است','شد','شده','می‌شود','می‌کند','میکند','کرد','کرده','برای','از','به','در','با','که','این','آن','یک',
  'های','ها','را','بر','تا','طبق','گزارش','اعلام','خبر','جدید','آخرین','رمزارز','کریپتو','بازار','شرکت',
]);

const IMPORTANT_LOCAL_TERMS = new Set([
  'bitcoin','btc','بیتکوین','بیت‌کوین',
  'ethereum','eth','اتریوم',
  'solana','sol','سولانا',
  'xrp','ripple','ریپل',
  'stablecoin','stablecoins','استیبلکوین','استیبل‌کوین',
  'mica','clarity','genius',
  'sec','cftc','fed',
  'binance','coinbase','gemini','bybit','okx',
  'blackrock','strategy','microstrategy','mstr','sharplink',
  'travel','rule','license','licensing','regulation','law',
  'taiwan','australia','poland','dubai','eu','europe',
]);

const LOCAL_ENTITY_PATTERNS: Array<[string, RegExp[]]> = [
  ['taiwan', [/taiwan/i, /تایوان/]],
  ['australia', [/australia/i, /استرالیا/]],
  ['travel_rule', [/travel\s*rule/i, /قانون\s*سفر/]],
  ['sharplink', [/sharplink/i, /شارپلینک/i]],
  ['ethereum', [/\beth\b/i, /ethereum/i, /اتریوم/]],
  ['stablecoin', [/stablecoin/i, /استیبل[\s‌-]*کوین/]],
  ['exchange', [/exchange/i, /صرافی/]],
  ['licensing', [/licens/i, /مجوز/]],
  ['regulation', [/regulat/i, /\blaw\b/i, /قانون|قانونی|چارچوب/]],
  ['mica', [/\bmica\b/i, /MiCA/i]],
  ['clarity_act', [/clarity/i, /کلریتی|Clarity/i]],
  ['binance', [/binance/i, /بایننس/]],
  ['coinbase', [/coinbase/i, /کوین[\s‌-]*بیس/]],
  ['blackrock', [/blackrock/i, /بلک[\s‌-]*راک/]],
  ['jpmorgan', [/jp\s*morgan/i, /جی[\s‌-]*پی[\s‌-]*مورگان/]],
  ['solana', [/solana/i, /سولانا/]],
  ['defi', [/\bdefi\b/i, /دیفای/]],
  ['arthur_hayes', [/arthur\s*hayes/i, /آرتور\s*هیز|هیز/]],
  ['token_unlock', [/token\s*unlock/i, /آزادسازی\s*توکن|توکن.*آزاد/]],
  ['etf', [/\betf\b/i, /صندوق/]],
];
...
function parseDuplicateLocalThreshold(env: Env): number {
  const raw = Number((env as any).DUPLICATE_AI_JUDGE_LOCAL_THRESHOLD ?? 0.34);
  return Number.isFinite(raw) ? Math.max(0.15, Math.min(0.9, raw)) : 0.34;
}

function parseDuplicateLocalPriorsPerItem(env: Env): number {
  const raw = Math.floor(Number((env as any).DUPLICATE_AI_JUDGE_LOCAL_PRIORS_PER_ITEM ?? 5));
  return Number.isFinite(raw) ? Math.max(1, Math.min(12, raw)) : 5;
}
...
function tokenizeLocal(value: unknown): Set<string> {
  const text = normalizeLocalText(value);
  const out = new Set<string>();
  for (const token of text.split(' ')) {
    if (token.length <= 2) continue;
    if (LOCAL_STOP_TERMS.has(token)) continue;
    out.add(token);
  }
  return out;
}

function extractLocalNumbers(value: unknown): Set<string> {
  const text = normalizeLocalText(value);
  const out = new Set<string>();
  const matches = text.match(/\$?\d+(?:[,.]\d+)*(?:\.\d+)?\s*(?:b|bn|m|k|btc|eth|sol|xrp|٪|%|هزار|میلیون|میلیارد)?/gi) ?? [];
...
function extractLocalEntities(value: unknown, extra: unknown[] = []): Set<string> {
  const raw = [String(value ?? ''), ...extra.map(v => String(v ?? ''))].join(' ');
  const normalized = normalizeLocalText(raw);
  const out = new Set<string>();

  for (const [entity, patterns] of LOCAL_ENTITY_PATTERNS) {
    if (patterns.some(pattern => pattern.test(raw) || pattern.test(normalized))) {
      out.add(entity);
    }
  }

  for (const token of tokenizeLocal(raw)) {
    if (IMPORTANT_LOCAL_TERMS.has(token)) out.add(token);
  }

  for (const item of extra) {
    const key = cleanLocalKey(item);
    if (key && key.length >= 3 && !LOCAL_STOP_TERMS.has(key)) out.add(key);
  }
...
function jaccard(a: Set<string>, b: Set<string>): number {
  ...
  return intersectionSize(a, b) / (a.size + b.size - intersectionSize(a, b));
}
...
function candidateLocalSignal(candidate: DuplicateAiJudgeCandidate): LocalDuplicateSignal {
  ...
  return {
    ...
    tokens: tokenizeLocal(combined),
    entities: extractLocalEntities(combined, extra),
    numbers: extractLocalNumbers(combined),
  };
}
...
function priorLocalSignal(prior: DuplicateAiJudgePrior): LocalDuplicateSignal {
  ...
  return {
    ...
    tokens: tokenizeLocal(combined),
    entities: extractLocalEntities(combined, [prior.storyKey, prior.eventType, prior.canonicalDate]),
    numbers: extractLocalNumbers(combined),
  };
}
...
function scoreLocalDuplicateCandidate(current: LocalDuplicateSignal, prior: LocalDuplicateSignal): { score: number; reasons: string[] } {
  ...
  const tokenSimilarity = jaccard(current.tokens, prior.tokens);
  if (tokenSimilarity >= 0.18) {
    score += Math.min(0.34, tokenSimilarity * 0.75);
    reasons.push(`token_overlap:${tokenSimilarity.toFixed(2)}`);
  }

  const entityOverlap = intersectionSize(current.entities, prior.entities);
  if (entityOverlap > 0) {
    score += Math.min(0.36, entityOverlap * 0.12);
    reasons.push(`entity_overlap:${entityOverlap}`);
  }

  const numberOverlap = intersectionSize(current.numbers, prior.numbers);
  if (numberOverlap > 0) {
    score += Math.min(0.22, numberOverlap * 0.11);
  ...
  // Broad market/theme overlap without entities is usually not enough. Humans
  // made "both mention crypto regulation" a duplicate detector, and now here we are.
  if (entityOverlap === 0 && tokenSimilarity < 0.34 && !current.storyKey) {
    score = Math.min(score, 0.28);
  }

  // Very strong entity + number overlap catches cases like SharpLink 10,000 ETH /
  // 886,725 ETH even when text hashes and tweet URLs differ.
  if (entityOverlap >= 2 && numberOverlap >= 1) {
    score = Math.max(score, 0.5);
    reasons.push('strong_entity_number_match');
  }
...
export async function runDuplicateAiJudgeForSurvivors(
  ...
  // Cost guard: if the local semantic key layer cannot find a plausible match,
  // do not spend a Claude call. Claude is the judge, not the bouncer, because
  // apparently even APIs need management hierarchy now.
